...
On this page, the analysis workflow presented in the main text is applied on each of the four example datasets on the protein group level. Additionally, two more datasets were downloaded and the same analysis are performed on both precursor- and protein-levels. 
...
```{r}
scProt_dpcFit <- dpc(diannSubset, nuis = scProt$nuis, b1.upper = Inf)
...
```{r, fig.width = 6, fig.height = 4}
ddaPlasmaProt$devPlot + ylim(0.90, 1)
```
...
# Supplmentary dataset: Sydney heart bank data
...
devPlot1 <- res[[1]]$devPlot + 
  ylim(0.90, 1) + ggtitle("Precursor-level")
devPlot2 <- res[[2]]$devPlot + 
  ylim(0.90, 1) + ggtitle("Protein-level")
...
# Supplmentary dataset: UPS1 spiked in yeast extract
...
With estimated parameters for the fitted d on precursor-level data being:
